<template>
  <aside class="static-page__sidebar">
    <div class="static-page__sidebar-title">{{$t('footer.navigation.info')}}</div>
    <nav class="static-page__nav" :class="{'visible': navStatus}">
      <ul v-if="navData.length > 0">
        <li
          v-for="link in navData"
          :key="link.id"
          :class="{'active': localePath(`/docs/${link.semanticId}`) == $route.path}"
          >
          <nuxt-link :to="{path: localePath(`/docs/${link.semanticId}`)}">
            {{link.title}}
          </nuxt-link>
        </li>
      </ul>
...
export default {
  ...
  async fetch() {
    try {
      ...
      this.navData = data.result.filter(x => x.sectionTypeId === 2).map(item => ({
        title: item.title,
        id: item.id,
